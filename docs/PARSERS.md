...
Most Tree-sitter-backed languages here use grammars from the `tree-sitter` or `tree-sitter-grammars` GitHub orgs.
Exceptions are expected when there is no official grammar and the selected grammar is viable, maintained, and documented in `internal/languages/catalog.go`.
Current exceptions:

- Nix is vendored from `nix-community/tree-sitter-nix`.
- GDScript is vendored from `PrestonKnopp/tree-sitter-gdscript`.
...
1. Pick the official upstream grammar repository and version, or a documented community grammar when no official grammar exists.
