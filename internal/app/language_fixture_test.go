...
func TestLanguageFixturesFollowStandardContract(t *testing.T) {
	...
	cases := []languageFixtureCase{
		...
		{
			name:             "gdscript",
			fixtureDir:       "tests/testdata/languages/gdscript",
			filePath:         "basic.gd",
			expectedLanguage: "gdscript",
			topName:          "PlayerController",
			topKind:          "class",
			childName:        "login",
			childKind:        "method",
			query:            "login",
			queryKind:        "method",
			queryQualified:   "PlayerController.login",
			showContains:     "func login(user: String) -> bool:",
		},
		{
			name:             "csharp",
